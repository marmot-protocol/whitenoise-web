...
- Design-system use is mandatory for every UI change. Reuse existing typography roles, colors, spacing and relationship tokens, icons, and primitives. If a requested result needs something outside the established system, explain the specific difference and ask the user before implementing it. Do not introduce a new token, role, icon, visual primitive, arbitrary value, or one-off override, or change established token values, without explicit approval for that change. Ordinary composition using existing system elements can proceed without further approval.
...
- Immediate state cuts only. Copy changes icon for three seconds. Shared footer remains on every route; the temporary Design system link is disabled at the user's request.
- Latest donation presentation supersedes the historical donation styling below: a single muted Surface containing “Support the work.” in the shared section role and ink tone, its explanatory paragraph in the shared muted body role, and the payment options below. Do not add a separate white introduction or “Make a donation” heading. Lightning and Bitcoin use the shared heading role in ink, compact spacing, and text Donate links. Both full addresses remain visible and selectable at all times, without truncation or disclosure controls. Each Donate link follows its own address with the shared fixed gap; the longer Bitcoin address must not push down Lightning’s Donate link. Copying always uses the full address. Preserve payment URI values and accessible labels.
- Desktop More shows the active destination as “More / FAQ”, “More / Agents”, “More / For developers”, etc., with a persistent underline. Nested routes retain their parent navigation state; MDK documentation routes belong to For developers, and Marmot specification routes belong to Marmot Protocol. More lists Agents, For developers, Marmot Protocol, then FAQ, followed by the legal links. Documentation pages omit the standalone Marmot Protocol resource link and the “On this page” disclosure. Keep the main navigation destinations; full specification links in guide content go directly to GitHub. Long-form documentation, blog posts, essays and policy text share the responsive section gap and heading/body gap, including subsection headings.
- The Build page is a curated “Build with Marmot” guide for developers building messaging clients. Keep the Rust quick start, library overview, protocol basics and client flow, with Libraries and Apps in separate repository columns. Agent setup belongs on `/agents`. The latest simplified-guide direction supersedes the raw README presentation: `/docs/mdk/README.md` redirects to `/build#mdk`, and full MDK documentation is linked on GitHub.
- Agents, For developers and Marmot Protocol use the shared sticky guide sidebar on the right of the content, with Getting started first and an underlined current guide. Desktop links use the UI line height with 4px vertical padding. The area title appears above the sidebar links, separated by a fine rule and shared 16px spacing. Inactive guide links are muted; only the active link is ink and underlined. Each is a self-contained, short guide on one scrolling page, with local sidebar anchors and scroll-based selection. Do not embed full upstream documentation or add links to other documentation pages within the prose. Clearly labeled full-documentation links go directly to GitHub. Legacy runtime routes redirect to their Agents section; deeper documentation routes redirect to the exact source on GitHub. At 900px and below it becomes a labeled native guide selector. Keep the main header unchanged pending review; do not promote these routes out of More yet. The sidebar replaces the horizontal runtime links.
- Homepage feature sections use one short, plain-language paragraph each, aimed at everyday users. Keep benefits clear; protocol names, key handling, runtime lists and relay configuration belong in the guides.
- Privacy Matters ends with a prominent “Read our Privacy Policy” link in the shared heading role and a short explanation of the policy. Do not include a Canary link in this closing section.
- Blog entries always include an image. Missing or failed images use the canonical white WN logo centered on the existing ink background, in both the listing and article header. Preserve the original colors of published blog images; do not apply grayscale filters.
- Blog articles have one “Back to blog” link at the top, using a registry left chevron and muted text. No default underline; underline the label on hover. Do not repeat the link at the bottom.
