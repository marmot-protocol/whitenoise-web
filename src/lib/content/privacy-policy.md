...
**"Usage and telemetry"** means the optional sharing of product analytics and telemetry described in Section 6. It is off by default and is controlled by a single choice in the app.

**"Group diagnostic log"** means an optional local diagnostic file recording protocol-level group and message operations, used to investigate delivery or group-state problems. Group diagnostic logging is off by default, has its own separate consent, and is described in Section 7.
...
**It is off by default.** Nothing described in this section is collected or transmitted unless you make an explicit, affirmative choice to turn on usage and telemetry in the app. Declining costs you no functionality. If the scope of what would be shared changes materially, the app asks again rather than carrying a previous choice forward.

**One choice, two streams.** A single in-app setting controls two independent streams, both sent to servers IPF operates. On iOS, this is **Share usage and telemetry**, and it applies to every profile on the device:
...
- **Telemetry,** sent to an IPF-hosted OpenTelemetry collector (`otlp.ipf.dev`), recording aggregate performance and reliability measurements.
...
**Withdrawal and retention.** You can turn usage and telemetry off at any time in the app settings. Doing so stops both streams, discards anything queued on your device that has not yet been sent, and rotates the telemetry installation identifier. Turning sharing off does not delete data already received by our servers.

Usage analytics records are scheduled for automatic deletion 180 days after collection. Telemetry is retained for 90 days.
...
## 7. Group Diagnostic Logs (Opt-In)

Separately from usage and telemetry, White Noise can record a detailed group diagnostic log to help investigate difficult protocol problems — messages that fail to arrive, or a group whose cryptographic state has diverged between devices.

**Group diagnostic logging is off by default and has its own separate consent.** Turning on usage and telemetry does not turn on group diagnostic logging, and turning on group diagnostic logging does not turn on usage and telemetry.

**iOS automatic group diagnostic-log sharing.** Turning on **Share group diagnostic logs** enables local recording and automatic uploads to IPF’s Goggles service for every profile on the device. It can upload eligible logs already stored locally. You do not approve each file separately. Turning sharing off stops new recording and prevents new automatic upload batches; a batch already in progress may finish. It does not delete existing local or uploaded logs.

**Android automatic technical-log sharing.** Android’s **Share technical logs** control records an append-only file locally and automatically sends eligible group diagnostic log files to Goggles, operated by IPF. This covers every profile on that installation and can include files recorded before the latest opt-in. A confirmation identifies the recipient and sensitive data before sharing is enabled. Updated Android builds require a fresh acknowledgement instead of treating an older local-recording choice as permission for automatic uploads. Cancel leaves sharing off. Turning sharing off stops new recording and prevents new upload batches. A batch already in progress may finish uploading its files. Turning sharing off does not delete files already stored locally or uploaded.

**Manual export and other clients.** A client may also let you create a copy of local group diagnostic logs and choose its recipient through the operating system’s share controls. Such an export is a separate action. In clients that offer local-only recording, the files remain on the device until you explicitly export them.

**Group diagnostic logs are more sensitive than the information described in Section 6, and they are not anonymized.** For each protocol operation, a log may record the operation and its outcome, timestamps, group identifiers, message and event identifiers, group epoch numbers, the relay addresses and subscriptions involved, an identifier for the app installation, and a hashed reference derived from your account identifier rather than the account identifier itself. Logs may also describe membership changes using hashed references. iOS uploads include the hardware model, platform, and app version.

**Group diagnostic logs do not contain** the decrypted content of any message, plaintext group state values, your private key or any other key material, or your full account identity.

When a group diagnostic log reaches IPF through automatic sharing or manual export, IPF can see metadata about your groups: that particular groups and messages exist, when operations on them happened, and which relays were involved. IPF still cannot see what anyone said, and the log does not identify group members by name or public key. Uploaded group diagnostic logs are treated as sensitive, access is restricted to the personnel investigating the reported problem, and they are deleted from our servers after 30 days.

**Clearing logs and requesting deletion.** On iOS, **Clear Diagnostic Logs** deletes local diagnostic files for all profiles on the device. It does not delete uploaded copies or turn off recording and automatic uploads. To request deletion of uploaded logs held by IPF, email [privacy@ipf.dev](mailto:privacy@ipf.dev) with the subject “White Noise data deletion”, as described in [Section 14](#section-14). Local clearing, disabling sharing, and requesting server-side deletion are separate actions.
...
To remove an identity from the iOS app, open **Settings → Sign Out**, leave **Wipe Data From This Device** enabled, enter the displayed profile name exactly, and confirm **Sign Out**. This permanently removes that profile and its local data from the device; previous chats will not return when you sign back in. Save any backup you intend to keep first. Signing out with the wipe option disabled keeps the profile and its local data on the device. Local removal does not request deletion of data already uploaded to IPF; use the email process above for that.

A Nostr identity is a cryptographic keypair that can also be used in other apps. Removing it from White Noise does not destroy copies of the key on other devices or erase events and encrypted media held by independent relays, servers, or recipients. IPF cannot guarantee deletion from those independent systems. We can process requests for identifiable records on infrastructure we control, including support correspondence and submitted group diagnostic logs. Usage analytics expires after 180 days; because its records are not linked to your Nostr identity, we generally cannot identify individual records for deletion. Turning usage sharing off stops future collection. Other retention limits are described in Sections 6, 7, and 13.
...
Usage and telemetry records are subject to a similar limit. They contain no identifier that IPF can link to you, so IPF generally cannot locate "your" records in order to provide access to them, correct them, or delete them individually. The property that protects your privacy also prevents us from identifying your data. You can withdraw consent at any time in the app settings, which stops future usage and telemetry collection. Group diagnostic-log sharing has its own control and withdrawal behavior, described in Section 7.
